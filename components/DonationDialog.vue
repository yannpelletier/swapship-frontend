<template>
  <v-dialog v-model="valueMid" transition="fade-transition" max-width="1000">
    <v-card>
      <v-card-title> Donate to the creators of the ecosystem </v-card-title>
      <v-card-subtitle class="mt-1">
        This ecosystem is being fueled exclusively through voluntary
        contributions. If you can afford it or if SwapShip or other RTC coins
        have made you a lot of money, please consider donating to one (or many)
        of these fine folks.
      </v-card-subtitle>
      <v-card-text class="vh-center">
        <v-row>
          <v-col
            v-for="(target, index) in donationTargets"
            :key="index"
            cols="12"
            sm="6"
            md="4"
