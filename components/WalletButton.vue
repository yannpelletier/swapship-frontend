...
export default {
  ...
  computed: {
    ...mapGetters('accounts', ['accountIsConnected', 'connectedAccount', 'connectedAccountAbbreviated'])
  },
  methods: {
    ...mapActions('accounts', ['refreshAccount', 'viewOnEtherscan']),

    async connectWallet () {
      await window.ethereum.request({ method: 'eth_requestAccounts' })
      await this.refreshAccount(() => {
        this.$store.dispatch('updateData', RefreshType.Integral)
      })
    },
    openWalletDialog () {
