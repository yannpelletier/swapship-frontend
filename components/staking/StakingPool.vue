<template>
  <v-expansion-panel class="mb-5 pool-header" :style="headerStyle">
    <v-skeleton-loader
      ref="loader"
      :style="loaderStyle"
    ...
    >
      <v-expansion-panel-header v-if="!pool.isError" :class="{'pool-irrelevant': !hasClaimable}">
        <v-row>
          <v-col cols="12" md="3" order="1" order-md="2" class="text-center">
            <v-row class="font-weight-medium title">
              {{ pool.name }}
              <v-card v-if="pool.multiplier > 0" class="mx-2 title" :color="multiplerColor" width="45" height="30">
                x{{ pool.multiplier }}
              </v-card>
            </v-row>
            <v-row v-if="pool.type === PoolType.LIQUIDITY" class="subheading font-weight-light">
        ...
        <template v-if="pool.isActive">
          ...
          <v-col cols="12" md="6" class="py-6">
            <template v-if="hasBalance">
              ...
              <v-row class="vh-center">
                <v-btn v-if="pool.type === PoolType.LIQUIDITY" color="primary" class="mx-2" @click="openGetLink">
                  Get LP Tokens
                  <v-avatar class="ml-2" size="20">
                    <v-img :src="pool.token.exchange.image" />
                  </v-avatar>
                </v-btn>
                <v-btn v-if="pool.type === PoolType.VAULT" color="primary" class="mx-2" @click="openGetLink">
                  Buy on Uniswap
                  <v-avatar class="ml-2" size="20">
                    <v-img :src="pool.token.image" />
                  </v-avatar>
                </v-btn>
                <v-btn v-if="stakeInputUnderApprovedAmount" :disabled="stakeInputZero" color="primary" class="mx-2" @click="stakeInPool">
                  Stake
                </v-btn>
            ...
            <template v-else>
              <v-card rounded height="100%" min-height="150" class="vh-center font-weight-light" color="#00000066">
                <v-col cols="10">
                  <v-row class="vh-center">
                    No tokens to stake
                  </v-row>
                  <v-row class="vh-center">
                    <v-col v-if="pool.type === PoolType.LIQUIDITY" cols="auto" class="vh-center">
                    ...
                    <v-col v-if="pool.type === PoolType.VAULT" cols="auto" class="vh-center">
                      <v-btn color="primary" @click="openGetLink">
                        Buy on UniSwap
                        <v-avatar class="ml-2" size="20">
                          <v-img :src="pool.token.image" />
                    ...
                    <v-col v-if="!stakeInputUnderApprovedAmount" cols="auto" class="vh-center">
          ...
          <v-col cols="12" md="3" class="py-6">
            <template v-if="hasStaked">
              ...
              <v-row>
                <v-spacer />
                <v-btn color="primary" :disabled="unstakeInputZero" @click="withdrawAndClaimRewards">
                  Unstake and claim
                </v-btn>
                <v-spacer />
              </v-row>
            ...
            <template v-else>
              <v-card rounded height="100%" min-height="150" class="pa-2 font-weight-light vh-center" color="#00000066">
          ...
          <!-- CLAIMABLE REWARDS -->
          <v-col cols="12" md="3" class="py-6">
            <v-card rounded height="100%" min-height="150" class="pa-2 vh-center" color="#00000066">
              <div v-if="hasClaimable">
                <div class="font-weight-bold">
                  Rewards
                </div>
                <div v-for="(reward, index) in pool.rewards" :key="index" class="font-weight-light ">
                  ...
                  {{ reward.tokenTicker }}
                  <v-tooltip v-if="reward.note" top color="accent" max-width="400">
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                        class="mx-2"
                        icon
                ...
                <div class="vh-center mt-4">
                  <v-btn color="primary" @click="claimRewards">
                    Claim
                  </v-btn>
                </div>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      unstake: new BigNumber('0').shiftedBy(this.pool.token.decimals).toString(),
      multiplierRarityMap: {
        1: {
  ...
  computed: {
    ...
    formatValue () {
      return (value, decimals) => {
        return new BigNumber(value).shiftedBy(-decimals).toPrecision(3).toString()
      }
    },
    formatPrice () {
    ...
    stakedValueStr () {
      if (this.hasStaked) {
        const decimal = new BigNumber(this.pool.tokenStaked).shiftedBy(-this.pool.token.decimals).toFormat(6).toString()
  ...
  methods: {
    injectShineAnimation () {
      const interval = Math.random() * (SHINE_MAX_INTERVAL - SHINE_MIN_INTERVAL) + SHINE_MIN_INTERVAL
      if (this.$refs.loader) {
    ...
    withdrawAndClaimAll () {
      Pool.dispatch('withdrawAndClaimRewards', { pool: this.pool, amount: this.pool.tokenStaked })
    },
    withdrawAndClaimRewards () {
      Pool.dispatch('withdrawAndClaimRewards', { pool: this.pool, amount: this.unstake })
    },
    claimRewards () {
...
.pool-header {
  ...
  background: linear-gradient(90deg, rgba(31,30,54,1) 0%, rgba(30,32,37,1) 30%, rgba(30,32,37,1) 50%, rgba(30,32,37,1) 70%, rgba(31,31,40,1) 100%);
}
</style>
